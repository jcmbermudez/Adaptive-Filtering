...
def classical_model(K0, R, varz, mu, N):
    ...
    for m in cp.arange(1,iterations):
        K = K - mu * (R @ K + K @ R) \
            + mu**2 * (cp.trace(R @ K) * R + 2 * R @ K @ R)\
            + mu**2 * varz * R + varq * cp.eye(N)
        msdt[m] = cp.trace(K)
...
#Jex = cp.zeros(iterations)          # initialize Jex vector
#Jex[0] = Lambdas.dot(msq)           # initial EMSE
#B = (mu*varz*cp.ones(N))/(2*cp.ones(N) - mu*Lambdas)
#C = msq - B
#for n in cp.arange(1,iterations):
#    msq = B + ((cp.ones(N) - mu*Lambdas)**(2*n))*C
...
SystemResponse = cp.array(cp.random.randn(N))

# Normalization of the system response
SystemResponse = SystemResponse/cp.sqrt(SystemResponse.dot(SystemResponse))
...
plt.figure()
t = np.arange(N)
markerline, stemlines, baseline = plt.stem(t,SystemResponse.get())
...
#X0 = cp.sqrt((1-alpha**2)*varx)*cp.random.normal(0,1,size=iterations+N-1)
...
liner = cp.array(varx*(alpha**(cp.arange(N))))
# Input autocorrelation matrix
R = cplinalg.toeplitz(liner)
...
la, S = linalg.eigh(R.get())
#print("eigenvalues:",cp.abs(la), '\n')
#print("eigenvectors: ", "\n", S, '\n')
spread = cp.abs(cp.max(la))/cp.abs(cp.min(la))
...
mse = cp.zeros(iterations)            # Mean square error
emse = cp.zeros(iterations)           # Excess mean square error
msd = cp.zeros(iterations)            # Mean square deviation
msdtheo = cp.zeros(iterations)        # Theoretical MSD(n)
emsetheo = cp.zeros(iterations)        # Theoretical EMSE(n)
K0 = cp.outer(SystemResponse,SystemResponse)  # Matrix K(0)
...
for r in cp.arange(runs):                     # loop for realizations
    SystemResponsen = SystemResponse          # Updates the channel response
    W = cp.zeros(N)                           # initializes W(0)
    
    # Input driving noise generation
    X0 = cp.sqrt((1-alpha**2)*varx)*cp.random.normal(0,1,size=iterations+N)
    X1 = cp.zeros(iterations+N)            # initializes vector of input samples
    X1[0] = X0[0]
    for k in cp.arange(1,iterations+N):    # AR(1) model
        X1[k] = alpha*X1[k-1] + X0[k]
        
    # Generation of the measurement noise sequence
    noise = cp.sqrt(varz)*cp.random.randn(iterations+N)
    
    # Adaptive algorithm
    for n in cp.arange(iterations):
        ...
        SystemResponsen = SystemResponsen + cp.sqrt(varq) * cp.random.randn(N)
...
msedb = 10 * cp.log10(mse)                    # MSE in dB
emsedb = 10 * cp.log10(emse)                  # EMSE in dB
msddb = 10 * cp.log10(msd)                    # MSD in dB
...
plt.plot(t,emsedb.get(),label='simulations'), plt.grid()    
...
plt.figure()
plt.plot(t,msddb.get(),label='simulations'), plt.grid()    
